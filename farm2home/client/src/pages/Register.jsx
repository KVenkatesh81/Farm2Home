...
export default function Register() {
  const [form, setForm] = useState({ name:'', email:'', password:'', role:'buyer', licenceNumber:'', location:'', phone:'' })
          ...
          <div className="flex gap-2">
            {['buyer','farmer','transport'].map(r => (
              <button type="button" key={r} onClick={() => setForm({...form, role: r})}
                className={'flex-1 py-2 text-sm rounded-lg border capitalize transition-all ' + (form.role === r ? 'bg-green-600 text-white border-green-600' : 'border-gray-200 text-gray-600 hover:bg-gray-50')}>
                {r}
              </button>
            ))}
          </div>
